class VietQR {
    ...
    formatLen(str) {
        ...
        return len < 10 ? `0${len}` : len.toString();
    }

    setTransactionAmount(money) {
        if (!money) return this;
        const val = money.toString();
        this.transactionAmount = `54${this.formatLen(val)}${val}`;
        return this;
    }

    setBeneficiaryOrganization(bin, accountNo) {
        this.bin = bin ? bin.toString() : "";
        const acquier = `00${this.formatLen(bin)}${bin}`;
        const consumer = `01${this.formatLen(accountNo)}${accountNo}`;
        const tag01Value = `${acquier}${consumer}`;
        const tag01 = `01${this.formatLen(tag01Value)}${tag01Value}`;
        const id38Value = `${this.guid}${tag01}${this.serviceCode}`;
        this.consumerAccountInformation = `38${this.formatLen(id38Value)}${id38Value}`;
        return this;
    }

    setAdditionalDataFieldTemplate(content) {
        let tag08 = content ? `08${this.formatLen(content)}${content}` : "";
        let tag05 = (this.bin && this.bin.includes("971025")) ? `0515MOMOW2W21786354` : "";
        const tag62Value = `${tag05}${tag08}`;
        if (tag62Value) this.additionalDataFieldTemplate = `62${this.formatLen(tag62Value)}${tag62Value}`;
        return this;
    }
    ...
    build() {
        ...
        let crc = this.calcCRC(rawData).toString(16).toUpperCase().padStart(4, '0');
        return rawData + crc;
    }
}
...
async function runGenerator() {
    ...
    try {
        ...
        document.body.innerHTML = "Error: System could not verify bank list.";
        return;
    }
